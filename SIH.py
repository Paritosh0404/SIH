...
# Load the gesture classification model (replace this with your custom model path)
...
# Gesture labels (replace these with your custom gesture labels)
gesture_labels = ["Hello", "Thank You", "Please", "Yes", "No"]  # Modify to match your trained dataset
...
def voice():
    engine = pyttsx3.init()
    engine.say(text_box.get("1.0", END))  # Read the text box content
    engine.runAndWait()
...
# Add sound button
sound_button = tk.Button(win, text='Speak', command=voice, font=('Arial', 14), bg='#20262E', fg='#F5EAEA')
sound_button.place(x=width - 150, y=100)

# Add clear button to clear the sentence
def clear_text():
    text_box.delete("1.0", END)

clear_button = tk.Button(win, text='Clear', command=clear_text, font=('Arial', 14), bg='#20262E', fg='#F5EAEA')
clear_button.place(x=width - 150, y=150)
...
# Text box to display the constructed sentence
text_box = Text(win, height=2, width=50, font=('Arial', 18), bg='#F5EAEA', fg='#20262E')
text_box.place(x=520, y=750)
    ...
    while True:
        ...
        if result.multi_hand_landmarks:
            for hand_landmarks in result.multi_hand_landmarks:
                ...
                if(gesture_text==prev_text):
                    continue

                # Set the current gesture display in Tkinter
                CountGesture.set(gesture_text)

                # Append the recognized word to the text box to form a sentence
                current_sentence = text_box.get("1.0", END).strip()
                if current_sentence:
                    text_box.insert(END, " " + gesture_text)
                else:
                    text_box.insert(END, gesture_text)
                prev_text=gesture_text
